...
int main(int argc, char* argv[]) {
    ...
    auto sample_names = sc_loader.get_sample_names();
    ...
    // 初始化统计信息
    Statistics stats;
    // 样本信息初始化，避免无拆分READS时数据不输出
    for (auto& sample_name :sample_names){
        stats.sample_counts[sample_name] = 0;
        stats.corrected_counts[sample_name] = 0;
    }
    ...
    // 启动消费者线程
    std::thread consumer_thread(consumer, std::ref(queue), output_dir, sample_names, std::ref(finished));
