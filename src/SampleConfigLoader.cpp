...
bool SampleConfigLoader::load(const std::string& filepath) {
    ...
    while (std::getline(infile, line)) {
        ...
        if (groups.size() != samples.size()) {
             std::cerr <<  "different size of groups and samples: " << line << " continue anyway\n";
            // 条形码组和样本数量不匹配，忽略该行
            continue;
        }
        ...
        for (size_t i = 0; i < groups.size(); ++i) {
            ...
            sample_names.push_back(samples[i]);
        }
    }
    infile.close();
    return true;
...
const std::vector<std::string>& SampleConfigLoader::get_sample_names() const {
    return sample_names;
}
